...
export function buildNationalOverview(
  ...
  return {
    ...
    onTimePctBaseline: base ? r1(Math.max(0, Math.min(100, 100 - base.delayPct - base.canceledPct))) : null,
    delayPctBaseline: base ? r1(base.delayPct) : null,
    avgDelayMinBaseline: base ? Math.round(base.avgDelayMin) : null,
    delayDeltaPct,
...
export function buildAirlinePerformance(
  ...
  return AIRLINES.map((airline) => {
    ...
    return {
      ...
      avgDelayMin: m.avgDelayMin,
      baselineDelayPct: baseline?.delayPct ?? null,
      baselineOnTimePct: baseline?.onTimePct ?? null,
      baselineAvgDelayMin: baseline?.avgDelayMin ?? null,
      deltaDelayPct,
      live: m.live,
      band,
