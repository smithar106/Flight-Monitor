...
export function Airlines({ airlines }: { airlines: AirlinePerformance[] }) {
  ...
  const mostDelayed = useMemo(() => {
    ...
    return live.reduce((worst, a) => (a.delayPct! > (worst?.delayPct ?? -1) ? a : worst), live[0]);
  }, [airlines]);
    ...
    <div>
      {mostDelayed && (
        <div className="flex flex-wrap items-center gap-2 border-b border-line px-4 py-3 text-[0.8125rem]">
          ...
          <span className="font-mono text-severe">{fmtPct(mostDelayed.delayPct)} delayed</span>
          <span className="text-[0.6875rem] text-ink-faint">(live sample)</span>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1 border-t border-line px-4 py-2.5 text-[0.6875rem] text-ink-faint">
        <span>Live sample covers {airlines.filter((a) => a.live).length} of {airlines.length} carriers</span>
